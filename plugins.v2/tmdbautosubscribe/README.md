...
- 默认按热度排序，在同一日期窗口内优先拉取热门条目；可在“扫描参数”切换为按日期排序，以优先查看窗口内较新的条目。
- 默认开启“跳过非中文标题”：TMDB 返回的显示标题不含汉字时，条目只保留在候选明细并标注“标题缺少中文，已跳过”，不会进入订阅建议或剧集观察缓存；关闭该开关后按其他规则正常处理。
...
- `TMDB排序方式`：默认按热度排序；切换为按日期后，电影、新剧和近期播出剧分别按对应上映/首播/播出日期倒序拉取。无论哪种排序，扫描日期窗口和细分筛选条件都不变。
